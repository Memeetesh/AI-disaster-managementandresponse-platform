...
// CARTO's free basemap tiles, not tile.openstreetmap.org — baking a tile
// pyramid into a downloadable file is exactly the kind of bulk/redistributed
// use OSM's raw tile server blocks under its Tile Usage Policy
// (https://operations.osmfoundation.org/policies/tiles/); CARTO's tiles are
// still OSM-sourced data, served from an endpoint meant for this.
const TILE_URL =
...
const MAX_TILES = 110; // keep the file a few MB, stay well inside fair use
...
new Off("", { minZoom: ${MIN_ZOOM}, maxZoom: ${MAX_ZOOM}, tileSize: 256, attribution: "&copy; OpenStreetMap contributors &copy; CARTO" }).addTo(map);
...
<title>Aasha Setu — Offline Emergency Map</title>
<style>
  ...
  <footer>Map data © OpenStreetMap contributors, © CARTO. Generated by the Aasha Setu citizen app.</footer>
</div></body></html>`;
}
...
export async function downloadOfflineMap(input: OfflineMapInput): Promise<void> {
  ...
  a.href = url;
  a.download = "aasha-setu-offline-map.html";
